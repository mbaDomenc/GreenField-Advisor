...
_AGG_TTL = int(os.getenv("AI_AGGR_TTL_SECONDS", "900"))  
...
def _parse_dt(dt) -> Optional[datetime]:
    ...
    try: return datetime.fromisoformat(str(dt).replace('Z', ''))
    except: return None
...
def _days_since(dt: Optional[datetime], now: datetime) -> Optional[int]:
    if not dt: return None
    try: return max(0, (now - dt).days)
    except: return None

def _baseline_from_stage(stage: Optional[str]) -> int:
    s = (stage or "").lower()
    if s in ("iniziale", "initial", "semina", "trapianto"): return 2
    if s in ("medio", "mid", "crescita", "fioritura"): return 2
    if s in ("finale", "late", "maturazione", "raccolta"): return 3
    return 3
...
async def get_inputs(plant: Dict[str, Any], now: Optional[datetime] = None) -> Dict[str, Any]:
    """
    Aggrega profilo FAO, NASA POWER, Open-Meteo (Async) e Copernicus.
    """
    ...
    if key in _AGG_CACHE and not _expired(_AGG_CACHE[key]):
        ...
        # Qui chiamiamo il nuovo controller meteo che supporta i trend
        try:
            om = await weatherController.get_weather_data(lat=lat, lon=lng) or {}
        except Exception:
            om = {}
        ...
        # Recupero trend pioggia per passarlo avanti (fondamentale per LLM)
        rain_trend = om.get("rain_trend", [])
        ...
        # Fallback precipitazioni
        if precipDaily is None:
            if isinstance(rainNext24h, (int, float)):
                precipDaily = float(rainNext24h)
                fallbacks["precipDaily"] = "rainNext24h"

        # Fallback ET0 (Hargreaves)
        if et0 is None and temp is not None:
            # Stima grezza se mancano min/max
            et0 = compute_et0_hargreaves(lat, temp - 5, temp + 5, temp, now=now)
            if et0: fallbacks["et0"] = "hargreaves_est"
        ...
        if solarRadiation is None:
            ...
            if ra:
                solarRadiation = ra
                fallbacks["solarRadiation"] = "Ra_estimate"

        # Suolo
        soil_moisture_0_7 = soil.get("soilMoisture0to7cm") or om.get("soil_moisture")
        ...
        if soil_moisture_approx is None:
            ...
            fallbacks["soilMoistureApprox"] = "from_air_rh"
        ...
        value = {
            ...
            "weather": {
                ...
                "rainNext24h": float(rainNext24h) if rainNext24h is not None else 0.0,
                "soilMoistureApprox": float(soil_moisture_approx) if soil_moisture_approx is not None else None,
                "soilMoisture0to7cm": float(soil_moisture_0_7) if soil_moisture_0_7 is not None else None,
                
                "precipDaily": float(precipDaily) if precipDaily is not None else 0.0,
                "et0": float(et0) if et0 is not None else None,
                "solarRadiation": float(solarRadiation) if solarRadiation is not None else None,
            ...
            "raw": { "nasa": nasa, "openmeteo": om, "soil": soil }
        }
