...
// Liste invariate (copiale dal file precedente o lasciale così)
...
export default function PlantFormModal({
  ...
  const [geo, setGeo] = useState(() => {
    ...
    if (geoLat && geoLng) return { lat: geoLat, lng: geoLng, placeId: placeId || null, addrParts: {} };
    return null;
  });
  ...
  const handleFileChange = async (e) => {
    ...
    if (file) {
      ...
      setHealthStatus(''); setHealthAdvice('');
      setAnalyzing(true);
      try {
  ...
  const handleUploadClick = () => {
    if (preview) return;
    if (!selectedSpecies) {
        setErrors(prev => ({ ...prev, species: "Seleziona prima la specie!", image: "⚠️ Seleziona prima la specie!" }));
        return;
    }
  ...
  const validate = () => {
    ...
    if (!selectedSpecies) next.species = 'Seleziona una specie';
    if (!placeText) next.location = 'La posizione è obbligatoria';
    
    // 🟢 NUOVA VALIDAZIONE TERRENO
    if (!soil) next.soil = 'La tipologia di terreno è obbligatoria';
    
    if (!selectedFile && !preview) next.image = 'La foto è obbligatoria';
        ...
        <div className="overflow-y-auto p-6">
          <form onSubmit={handleSubmit} id="plantForm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* ALTRI CAMPI (Nome, Specie, Posizione identici) */}
                ...
                <div className="relative">
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            // Rimuovi errore se selezionato
                            if(errors.soil) setErrors(prev => ({...prev, soil: null}));
                        }}
                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 appearance-none bg-white ${errors.soil ? 'border-red-500' : 'border-gray-300'}`}
                    >
